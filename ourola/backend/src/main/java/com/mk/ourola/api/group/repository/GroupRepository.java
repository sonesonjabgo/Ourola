...
public interface GroupRepository extends JpaRepository<GroupDto, Integer> {
	...
	@Query(value = "SELECT c.id, c.name, c.file_path, c.file_extension FROM (select * from group_subscribe gs where gs.user_id= :fanUserId) s RIGHT JOIN group_channel c on s.group_id = c.id WHERE s.group_id IS NULL",
		nativeQuery = true)
